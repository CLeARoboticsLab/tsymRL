...
def _make_model(n_poles):
  ...
  xml_string = common.read_model('fixed_multipole.xml')
  if n_poles == 1:
    return xml_string
...
class Physics(mujoco.Physics):
  ...
  def bounded_position(self):
    ...
    return self.named.data.xmat[2:, ['zz', 'xz']].ravel()
...
class Balance(base.Task):
  ...
  def get_rev_reward(self, physics, action):
    ...
    return self._get_rev_reward(physics, action, sparse=self._sparse)